...
import rus from './hyphen/patterns/ru.js';
import eng from './hyphen/patterns/en-us.js';
import fs from 'fs';
...
export default class PrelumHtmlView {

    constructor(plugin, uri) {
        this.uri = uri;
        this.plugin = plugin;
        this.timers = [];
    ...
    setSourceLine(ln) {
        ...
        let line = ln - 1,
            _uri = this.plugin.getOutputUri(this.file.path, ['pre', 'prelum']),
            editor = this.plugin.getOutputPaneItem(_uri);

        if (editor) {
            editor.setCursorBufferPosition([ line, 0 ], { autoscroll: false });
        ...
        _uri.forEach(i => {
            if (isOpened) return;
            if (fs.existsSync(i)) {
                atom.workspace.open(i, { split: 'left', initialLine: line });
                isOpened = true;
            }
        });
