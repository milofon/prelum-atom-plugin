...
import specs from '../templates/spec';
...
const CLASSES = {
  ...
  publicator: prefix + '__publicator',
};
...
class PrelumBarView {
  constructor() {
            ...
            <div class="${prefix}">
                <div class="${CLASSES.status}">
                    ...
                    <div class="${CLASSES.status}-right">
                        <div class="loading loading-spinner-tiny ${
                          CLASSES.status
                        }-loader"></div>
                        <button class="btn icon icon-remove-close ${
                          CLASSES.status
                        }-close"></button>
                    </div>
                </div>
                <div class="${CLASSES.left}">
                    ...
                    <button class="btn ${
                      CLASSES.newSpec
                    } icon icon-file"></button>
                    <button class="btn ${
                      CLASSES.settings
                    } icon icon-gear"></button>
                    <button class="btn ${
                      CLASSES.publish
                    } icon icon-globe"></button>
                </div>
    ...
    let links = {
      ...
      publicator: element.find(`.${CLASSES.publicator}`),
    ...
    let genSpecWindow = () => {
      ...
      modal.find('.input-search').on('input', function(e) {
    ...
    // create event listeners
    {
      ...
      links.publicator.click(function() {
  ...
  updateValues(path) {
    ...
    links.autoInput[0].checked = links.translate[0].disabled =
  ...
  getParams(path) {
    if (!this.params[path]) {
      ...
      _p['format'] =
        this.links.formats.find('.selected').html() ||
  ...
  setColor(type, to = 'translate') {
    if (!type) return;

    this.clearTimers();

    let target = this.links[to],
      cls = 'btn-info btn-success btn-error btn-warning';
  ...
  enable() {
    ...
    const { element, links, params } = this;

    $(element)
      .find('input, button')
      .each((i, el) => {
        const translateButton = $(el).hasClass(CLASSES.translate);
        const translateOnBarActivated = params[src.fullPath]['autoTranslate'];

        el.disabled = translateButton && translateOnBarActivated;
      });
