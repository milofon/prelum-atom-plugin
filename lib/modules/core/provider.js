...
export default {
  ...
  async getSuggestions(request) {
    const scopes = request.scopeDescriptor.scopes.join();
    this.checkLinksChange(request);
    let s = [];
    if (scopes.indexOf('block.prelum') < 0) {
      s = this.getKeywords(request);
    } else if (scopes.indexOf('terminus') >= 0) {
      s = await this.getTerms(request);
    } else if (scopes.indexOf('bibliography') >= 0) {
      s = await this.getBibliography(request)
    }
  ...
  onDidInsertSuggestion({ editor, suggestion, triggerPosition: tp }) {
    ...
    if (suggestion.type === 'import') {
      let insert = function (text, pos, saveText = false) {
        let _pos = pos || [0, 0];
        let txt = '';

        if (saveText) {
          const editorText = editor.getText();
          const diffText = editorText
            .replace(text, '')
            .replace(/документ\([^)]*\)/, '');
          txt = text + '\n' + diffText;
        } else {
      ...
      insert(suggestion.snippet, null, true);
  ...
  async getTerms({ bufferPosition: bp, prefix }) {
    const termsUrl = atom.config.get('prelum.termsUrl');
    const pfx = encodeURIComponent(prefix);
    const haveKeyword = bp.column - 2 === prefix.length;

    if (!pfx || !termsUrl || (bp.column !== prefix.length && !haveKeyword))
      return [];

    const url = `${termsUrl}/terms?name=${pfx}&limit=6`;
    ...
    if (results) {
      return results.map(term => {
        ...
        const termDefs = term.definitions.map(item => {
          return item.definition + (item.source ? ' (' + item.source + ')' : '')
        }).join('\n')

        return {
          type: 'variable',
          displayText: term.name,
          text: `${haveKeyword ? '' : '= '}${term.name} - ${termDefs}`
        };
      });
    }
  ...
  async getBibliography({ bufferPosition: bp, prefix }) {
    const termsUrl = atom.config.get('prelum.termsUrl');
    const pfx = encodeURIComponent(prefix);
    const haveKeyword = bp.column - 2 === prefix.length;
    ...
    const url = `${termsUrl}/bibs?search=${pfx}&limit=6`;
    const response = await fetch(url);
    const results = await response.json();
    ...
    if (results) {
      return results.map(term => {
        return {
          type: 'variable',
          displayText: term.citation,
          text: `${haveKeyword ? '' : '= '}${term.label} - ${term.citation}`,
        };
      });
    }
  ...
  checkParam(_s) {
    return (
      _s.indexOf(preScopes.param) >= 0 || _s.indexOf(preScopes.bracket) >= 0
    );
  },
  ...
  getLinksData({ editor, bufferPosition: bp, prefix, legacyPrefix }) {
    let _completions = [],
      _start = [bp.row, 0],
    ...
    _result.forEach(r => {
      _completions.push({
        ...
        leftLabel: r.keyword,
        rightLabel: 'prelum',
        replacementPrefix: prefix || legacyPrefix,
        type: 'constant',
