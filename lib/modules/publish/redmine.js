...
export default function() {
  ...
  // get links
  {
    ...
    src.editor.scanInBufferRange(
      /(redmine-wiki|redmine-issue)\s\-\s(.+)/gm,
      metaRange,
      function(result) {
  ...
  if (links.length === 0) {
    log('info', {
      title: 'Нет ресурсов Redmine для публикации',
    });
    return;
  }
  ...
  function final() {
    ...
    plugin.bar.updateStatusPanel({
      title: `<span class="text-success">Опубликовано</span> - <b>${
        success.length
      }</b>&ensp;&ensp;<span${
        errors.length ? ' class="text-error"' : ''
      }>Ошибок</span> - <b>${errors.length}</b>&ensp;&ensp;Всего - <b>${
        links.length
      }</b>`,
      loader: false,
      close: true,
