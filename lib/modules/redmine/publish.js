...
export default function(prelum) {

    const src = prelum.getSourceInfo();

    if (src === 'notprelum') {
        prelum.message('error', { detail: 'Ожидается файл формата Prelum' });
        return;
    }

    prelum.dismissNotifications();
    ...
    if (links.length === 0) {
        notifications.addInfo('PRELUM: Нет ресурсов для публикации');
        return;
    }

    prelum.bar.showStatus();
